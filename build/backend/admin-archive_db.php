...
class Database extends Config
{
  ...
  public function archiveRecord($user_id, $origin, $key, $value)
  {
    ...
    $sqlArchive = "INSERT iNTO admin_archive (user_id, origin_table, data) VALUES (:user_id, :origin, :data)";
  ...
  public function recoverRecord($id)
  {
    ...
    $sqlFetch = "SELECT * FROM admin_archive WHERE id = :id";
    $stmtFetch = $this->conn->prepare($sqlFetch);
    ...
    $sqlDelete = "DELETE FROM admin_archive WHERE id = :id";
    $stmtDelete = $this->conn->prepare($sqlDelete);
  ...
  public function getTotalRows($query, $origin)
  {
    ...
    $sql = "SELECT COUNT(*) as total FROM admin_archive
  ...
  public function deleteRecord($id) {
    $sql = 'DELETE from admin_archive WHERE id = :id ';
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
      'id' => $id,
    ]);
