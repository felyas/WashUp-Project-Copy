...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tbody = document.getElementById('admin-archive-list');
  ...
  const fetchAll = async (page = 1, column = 'id', order = 'desc', query = '') => {
    ...
    const data = await fetch(`./backend/admin-archive_action.php?readAll=1&page=${page}&column=${column}&order=${order}&$query=${searchQuery}&origin=${originQuery}`, {
      method: 'GET',
    });
  ...
  const recoverData = async (archiveId) => {
    const data = await fetch(`./backend/admin-archive_action.php?recover=1&archive_id=${archiveId}`, {
      method: 'GET',
    });
  ...
  const deleteData = async (id) => {
    const data = await fetch(`./backend/admin-archive_action.php?delete=1&id=${id}`, {
      method: 'GET',
    });
    const response = await data.json();
    if (response.status === 'success') {
